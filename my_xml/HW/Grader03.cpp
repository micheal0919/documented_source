  ...
  std::string output_filename, int window_size)
{
  ...
  {
	  std::cout << "output_vec[] " << i << "th = " << output_vec[i] << std::endl;
  }
  for (size_t i = 0; i < 20; i++)
  {
	  std::cout << "user_output[] " << i << "th = " << user_output[i] << std::endl;
  }
  std::cout << "output_vec.size() = " << output_vec.size() << endl;
  std::cout << "user_output.size() = " << user_output.size() << endl;
